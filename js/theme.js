...
$('body').on("themeLoaded", function(){
    ...
    if($('#theme_video').length != 0)
    {
        $('#theme_video').mb_YTPlayer();
    }

    //Ripple

    if( $('#theme_background_ripple').length != 0 )
    {
        $('#theme_background_ripple').ripples({
            resolution:500,
            dropRadius:25,
            perturbance:0.03
        });
    ...
    //ScrollSpy
    $('a.theme_scrollspy[href^="#"]:not([href="#"]').on('click', function(event){
